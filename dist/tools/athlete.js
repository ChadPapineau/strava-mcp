...
exports.registerAthleteTools = registerAthleteTools;
...
function registerAthleteTools(server) {
...
Returns: Athlete profile including id, name, location, weight, FTP, summit status, and more.`,
        inputSchema: index_js_1.GetAthleteSchema,
        annotations: {
            readOnlyHint: true,
        ...
        try {
            ...
            const text = params.response_format === constants_js_1.ResponseFormat.JSON
                ...
                : (0, formatters_js_1.formatAthleteMarkdown)(athlete);
    ...
    // ── Get Athlete Stats ──────────────────────────────────────────────
    server.registerTool("strava_get_athlete_stats", {
...
Returns: Recent (4 week), YTD, and all-time totals for rides, runs, and swims.`,
        inputSchema: index_js_1.GetAthleteStatsSchema,
        annotations: {
            readOnlyHint: true,
            destructiveHint: false,
            idempotentHint: true,
        ...
        try {
            const client = (0, strava_client_js_1.getClient)();
            const stats = await client.get(`/athletes/${params.athlete_id}/stats`);
            const text = params.response_format === constants_js_1.ResponseFormat.JSON
                ? JSON.stringify(stats, null, 2)
                : (0, formatters_js_1.formatStatsMarkdown)(stats);
            return { content: [{ type: "text", text }] };
        }
        catch (error) {
            return {
    ...
    // ── Get Athlete Zones ──────────────────────────────────────────────
    server.registerTool("strava_get_athlete_zones", {
...
Returns: Heart rate zones (with custom flag) and power zones (if FTP is set).`,
        inputSchema: index_js_1.GetAthleteZonesSchema,
        annotations: {
            readOnlyHint: true,
            destructiveHint: false,
            idempotentHint: true,
        ...
        try {
            const client = (0, strava_client_js_1.getClient)();
            const zones = await client.get("/athlete/zones");
            ...
            if (zones.heart_rate) {
                lines.push(`## Heart Rate Zones ${zones.heart_rate.custom_zones ? "(Custom)" : "(Default)"}`, "");
