...
import {
  ...
  GetRouteStreamsSchema,
} from "../schemas/index.js";

function formatStreamSummary(streams: Record<string, { data: number[]; series_type: string; original_size: number; resolution: string }>): string {
...
Note: Streams can be very large. For full data, use JSON format. Markdown shows a statistical summary.`,
    ...
    async (params: z.infer<typeof GetActivityStreamsSchema>) => {
      try {
        const client = getClient();
        const streams = await client.get<Record<string, { data: number[]; series_type: string; original_size: number; resolution: string }>>(
          `/activities/${params.id}/streams`,
          {
            keys: params.keys,
        ...
        if (params.response_format === ResponseFormat.JSON) {
          return {
            content: [
              {
                type: "text",
                text: truncateIfNeeded(JSON.stringify(streams, null, 2)),
              },
            ],
          };
        }
...
Returns: Time-series data for the segment effort.`,
    ...
    async (params: z.infer<typeof GetSegmentEffortStreamsSchema>) => {
      try {
        const client = getClient();
        const streams = await client.get<Record<string, { data: number[]; series_type: string; original_size: number; resolution: string }>>(
          `/segment_efforts/${params.id}/streams`,
          { keys: params.keys, key_by_type: params.key_by_type }
        );
...
Returns: GPS track data (latlng, distance, altitude) for the route.`,
      inputSchema: GetRouteStreamsSchema,
      annotations: {
        readOnlyHint: true,
        destructiveHint: false,
        idempotentHint: true,
    ...
    async (params: z.infer<typeof GetRouteStreamsSchema>) => {
      try {
        const client = getClient();
        const streams = await client.get<Record<string, { data: number[]; series_type: string; original_size: number; resolution: string }>>(
          `/routes/${params.id}/streams`
        );
