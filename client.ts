...
const TOKEN_FILE = path.join(process.cwd(), ".strava_tokens.json");
...
export function apiError(err: unknown): string {
  if (axios.isAxiosError(err)) {
    ...
    if (s === 401) return `Auth error: ${msg}. Re-run the OAuth setup at http://localhost:${process.env.PORT || 3000}/setup`;
